...
import { ArrowLeft, ArrowRight, Truck, ShieldCheck, CreditCard, ChevronRight, X } from 'lucide-react';

export default function App() {
  // State
  const [view, setView] = useState<'home' | 'product' | 'archive' | 'checkout'>('home');
  ...
  const [priceMatchProduct, setPriceMatchProduct] = useState<Product | null>(null);
  ...
  const nextHeroCategory = () => setHeroCategoryIndex((prev) => (prev + 1) % heroCategorySlides.length);
  const prevHeroCategory = () => setHeroCategoryIndex((prev) => (prev - 1 + heroCategorySlides.length) % heroCategorySlides.length);

  const nextProjectSlide = () => setProjectSlideIndex((prev) => (prev + 1) % projectSlides.length);
  const prevProjectSlide = () => setProjectSlideIndex((prev) => (prev - 1 + projectSlides.length) % projectSlides.length);
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[45%_30%_25%] gap-6 mb-12 items-stretch overflow-hidden">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-between text-white">
                  <div className="max-w-lg space-y-4">
                    <div className="flex items-center gap-3">
                      <span className="bg-belims-accent text-white px-2 py-1 text-xs font-bold uppercase tracking-wider rounded-sm font-heading">{activeHero.tag}</span>
                      <span className="text-xs font-bold uppercase tracking-wide text-gray-200 font-heading">{activeHero.category}</span>
                    </div>
                    <div className="space-y-3">
                      <h2 className="text-4xl font-bold leading-tight font-heading">{activeHero.title}</h2>
                      <p className="text-lg text-gray-100 font-medium">{activeHero.subtitle}</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-3">
                      <button onClick={prevHeroCategory} className="h-10 w-10 rounded-full bg-white/15 border border-white/30 backdrop-blur flex items-center justify-center hover:bg-white/25 transition">
                        <ArrowLeft className="text-white" size={18} />
                      </button>
                      <button onClick={nextHeroCategory} className="h-10 w-10 rounded-full bg-white/15 border border-white/30 backdrop-blur flex items-center justify-center hover:bg-white/25 transition">
              ...
              <div className="lg:col-span-1 flex flex-col gap-4 min-h-[420px] h-full">
                ...
                >
                  ...
                  <div className="relative flex items-center justify-between px-4 py-3">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-wide text-belims-accent font-heading">DIY Project Ideas</p>
                      <h3 className="text-lg font-bold font-heading">{activeProject.title}</h3>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={prevProjectSlide} className="h-9 w-9 rounded-full border border-white/40 bg-black/30 flex items-center justify-center hover:bg-white/20 transition" aria-label="Previous project">
                        <ArrowLeft size={16} />
                      </button>
                      <button onClick={nextProjectSlide} className="h-9 w-9 rounded-full border border-white/40 bg-black/30 flex items-center justify-center hover:bg-white/20 transition" aria-label="Next project">
                        <ArrowRight size={16} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="relative p-4 space-y-3">
                    <p className="text-sm leading-relaxed text-white/90 max-w-md">{activeProject.description}</p>
                    <span className="text-belims-accent font-bold text-sm underline font-heading">{activeProject.linkText}</span>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 shadow-sm flex-1 flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-wide text-belims-blue font-heading">DIY Tips & Tricks</p>
                      <h4 className="text-lg font-bold text-gray-900 font-heading">Pro how-tos for this weekend</h4>
                    </div>
                    <ChevronRight className="text-belims-blue" size={18} />
                  </div>
                  <p className="text-sm text-gray-600">From laying bricks to wiring a plug, get expert advice for your next project.</p>
                </div>
              </div>
              ...
              <div className="lg:col-span-1 bg-belims-blue rounded-lg p-5 text-white flex flex-col relative overflow-hidden shadow-lg min-h-[420px] h-full">
                <div className="relative z-10">
                  ...
                  <h2 className="text-xl font-extrabold mb-1 font-heading tracking-tight">MyBelims</h2>
                  <h3 className="text-2xl font-extrabold mb-5 text-white font-heading tracking-tight">Pro Rewards</h3>

                  <div className="mb-6">
                    <p className="text-lg font-bold mb-2 font-heading">Build. Earn. Save.</p>
                  ...
                  <ul className="space-y-3 text-sm font-medium text-blue-50 mb-8">
                    <li className="flex gap-2 items-center"><div className="bg-white text-belims-blue rounded text-[10px] p-0.5 font-bold">%</div> Bulk Pricing.</li>
                    <li className="flex gap-2 items-center"><div className="bg-white text-belims-blue rounded text-[10px] p-0.5 font-bold">Job</div> Site Delivery.</li>
                    <li className="flex gap-2 items-center"><Truck size={14} /> Dedicated Support.</li>
                  </ul>
            ...
            <section className="mb-16">
              <div className="flex justify-between items-end mb-6 border-b border-gray-200 pb-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 font-heading">Top Rated Hardware</h2>
                  <p className="text-gray-500 text-sm mt-1 font-medium">Essential tools and materials for the job.</p>
                </div>
                <a href="#" className="text-belims-blue font-bold text-sm hover:text-belims-accent hover:underline flex items-center gap-1 mb-2 font-heading">
            ...
            <div className="bg-belims-gray rounded-xl p-8 mb-16 flex flex-col md:flex-row items-center justify-between gap-8 border border-gray-200">
              <div>
                ...
                <p className="text-gray-600 max-w-lg text-lg">Exclusive volume savings, dedicated support, and job site delivery for professionals.</p>
              </div>
