      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl flex flex-col transform transition-transform duration-300 animate-slideInRight">
        ...
        <div className="bg-blue-50 p-4 border-b border-blue-100 flex gap-3 items-center">
          <img
            ...
            loading="lazy"
            decoding="async"
            className="w-12 h-12 object-contain bg-white border rounded p-1"
            alt=""
          />
          <div>
        ...
        <div className="p-5 bg-white border-t shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-10">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-xs font-bold text-gray-500 mb-1">
              <span className={count >= 1 ? "text-belims-blue" : ""}>
                3% OFF (1 item)
              </span>
              <span className={count >= 2 ? "text-belims-blue" : ""}>
                5% OFF (2 items)
              </span>
              <span className={count >= 3 ? "text-belims-blue" : ""}>
                10% OFF (3+)
              </span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden flex">
              <div
                className={`h-full bg-belims-blue transition-all duration-300 ${count >= 1 ? "w-1/3" : "w-0 opacity-0"}`}
              ></div>
              <div
                className={`h-full bg-belims-blue transition-all duration-300 ${count >= 2 ? "w-1/3" : "w-0 opacity-0"}`}
              ></div>
              <div
                className={`h-full bg-belims-blue transition-all duration-300 ${count >= 3 ? "w-1/3" : "w-0 opacity-0"}`}
              ></div>
            </div>
          </div>
