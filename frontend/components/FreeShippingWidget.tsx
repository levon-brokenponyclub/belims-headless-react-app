...
export const FreeShippingWidget: React.FC<FreeShippingWidgetProps> = ({
  cartItems,
}) => {
        ...
        <div className="flex-1 w-full">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2 font-bold text-gray-800 font-heading">
              {isFree ? (
                <>
                  <CheckCircle className="text-green-500" size={20} /> You've
                  unlocked FREE Shipping!
                </>
              ) : (
                <>
                  <Truck className="text-belims-blue" size={20} /> Add{" "}
                  {formatCurrency(remaining)} for FREE Shipping
                </>
              )}
            </div>
