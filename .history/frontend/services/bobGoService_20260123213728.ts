...
const BOBGO_API_URL = import.meta.env.VITE_BOBGO_API_URL || 'https://api.bobgo.co.za/v2';
const BOBGO_API_KEY = import.meta.env.VITE_BOBGO_API_KEY;
const BOBGO_CHANNEL_ID = import.meta.env.VITE_BOBGO_CHANNEL_ID;

export const getShippingRates = async (params: ShippingQuoteParams): Promise<ShippingRate[]> => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([
        { service_code: 'eco', service_name: 'Economy Road', total_price: 150.00, expected_delivery_date: '3-5 Days' },
        { service_code: 'exp', service_name: 'Express Air', total_price: 350.50, expected_delivery_date: '1-2 Days' },
      ]);
    }, 1000);
  });
};
