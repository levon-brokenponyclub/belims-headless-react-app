...
export const SingleProduct: React.FC<SingleProductProps> = ({ product, addToCart, onBuyNow, onBack, onCompare, onPriceMatch, onProductClick, onBrandClick }) => {
  ...
  return (
    <div className="animate-fadeIn relative">
      ...
      {/* Fixed Breadcrumbs */}
      <div className="fixed left-0 right-0 bg-white/98 backdrop-blur-md border-b border-gray-200 z-50 px-6 py-4 shadow-sm" style={{ top: `${breadcrumbTop}px` }}>
      ...
      {/* Main Grid Layout */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-start relative" style={{ paddingTop: contentPaddingTop }}>

        {/* LEFT COLUMN: Sticky Image + Overlapping Sticky Control Box */}
        <div className={`lg:col-span-7 ${isMobile ? '' : 'sticky top-[190px] h-[calc(100vh-220px)]'} flex flex-col relative z-30`} style={{ paddingLeft: '0' }}>
              ...
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${(isStickyExpanded && showBuyBoxFull) ? 'max-h-[200px] opacity-100 mb-3 border-b border-gray-100 pb-3' : 'max-h-0 opacity-0'}`}>
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-extrabold text-belims-blue font-heading">{CURRENCY_SYMBOL}{product.price.toFixed(2)}</div>
                  </div>
                </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-8 pb-24 pt-0">
          ...
          <div className={`sticky ${isMobile ? 'top-[117px]' : 'top-[190px]'} z-20 bg-white/95 backdrop-blur py-3 -my-2 border-b border-gray-100`}>
      ...
      {isMobile && (
        <div className={`fixed left-0 right-0 bottom-0 z-[260] bg-white border-t border-gray-200 shadow-2xl px-4 py-3 transition-transform duration-300 ${showBottomCta ? 'translate-y-0' : 'translate-y-full'}`}>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 bg-belims-blue text-white font-bold text-sm h-11 rounded-lg shadow-md hover:bg-belims-light transition-all active:scale-95 font-heading flex items-center justify-center gap-2 disabled:opacity-50"
            >
              <ShoppingCart size={16} /> {product.stock > 0 ? 'Add' : 'Out of Stock'}
            </button>
            <button
              onClick={handleBuyNowAction}
              disabled={product.stock === 0}
