import React from "react";

const assets = {
  left: "https://i5.walmartimages.com/dfw/4ff9c6c9-57a4/k2-_d96a5c18-0407-4248-b751-cdef0aa2dc5e.v1.jpg?odnHeight=1316&odnWidth=1316&odnBg=&odnDynImageQuality=70",
  midTop:
    "https://i5.walmartimages.com/dfw/4ff9c6c9-b8d0/k2-_076bc213-34e2-4307-ae2f-9a0f228e776a.v1.jpg?odnHeight=584&odnWidth=1024&odnBg=&odnDynImageQuality=70",
  midBottomLeft:
    "https://i5.walmartimages.com/dfw/4ff9c6c9-5f8f/k2-_38b00c0b-ef8d-4dc3-b5a8-012d7b2424a6.v1.jpg?odnHeight=684&odnWidth=496&odnBg=&odnDynImageQuality=70",
  midBottomRight:
    "https://i5.walmartimages.com/dfw/4ff9c6c9-b62d/k2-_df0d84dd-1d18-43ee-b6ba-3ec3cb8b8901.v1.jpg?odnHeight=684&odnWidth=496&odnBg=&odnDynImageQuality=70",
  right:
    "https://i5.walmartimages.com/dfw/4ff9c6c9-a9a3/k2-_247dba7f-cee1-4774-a5ba-53b98431c1d5.v1.jpg?odnHeight=1316&odnWidth=770&odnBg=&odnDynImageQuality=70",
};

type HeroCardProps = {
  image: string;
  eyebrow?: string;
  title: string;
  cta: string;
  href?: string;
  variant?: "pill" | "link";
  contentClassName?: string;
  textColorClassName?: string;
  ariaLabel?: string;
};

function HeroCard({
  image,
  eyebrow,
  title,
  cta,
  href = "#",
  variant = "link",
  contentClassName = "",
  textColorClassName = "text-slate-900",
  ariaLabel,
}: HeroCardProps) {
  return (
    <div className="relative h-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <img
        src={image}
        alt={ariaLabel ?? title}
        className="absolute inset-0 h-full w-full object-cover"
        loading="lazy"
      />
      <div className="absolute inset-0 bg-white/25" />

      <div
        className={
          "relative z-10 flex h-full flex-col items-start justify-start p-5 md:p-6 " +
          contentClassName
        }
      >
        {eyebrow ? (
          <p
            className={
              "text-sm font-semibold leading-tight " + textColorClassName
            }
          >
            {eyebrow}
          </p>
        ) : null}

        <h3
          className={
            "mt-1 max-w-[18ch] text-balance text-2xl font-extrabold leading-tight md:text-3xl " +
            textColorClassName
          }
        >
          {title}
        </h3>

        <div className="mt-4">
          {variant === "pill" ? (
            <a
              href={href}
              aria-label={cta}
              className="inline-flex items-center rounded-full bg-white px-5 py-2 text-sm font-bold text-slate-900 shadow-sm hover:bg-white/90"
            >
              {cta}
            </a>
          ) : (
            <a
              href={href}
              aria-label={cta}
              className={
                "text-sm font-semibold underline underline-offset-4 hover:no-underline " +
                textColorClassName
              }
            >
              {cta}
            </a>
          )}
        </div>
      </div>
    </div>
  );
}

export default function Hero() {
  return (
    <section className="w-full">
      <div className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-8">
        <div className={"grid gap-4 md:gap-5 lg:grid-cols-[1.15fr_1fr_0.85fr]"}>
          <div className="lg:row-span-2">
            <div className="aspect-square">
              <HeroCard
                image={assets.left}
                eyebrow="OnePay CashRewards Card"
                title="Earn 3% cash back at Walmart"
                cta="Learn more"
                href="https://www.walmart.com/cp/9435689"
                variant="pill"
                textColorClassName="text-[#001E60]"
                contentClassName="w-full md:w-[80%]"
                ariaLabel="OnePay CashRewards Card"
              />
            </div>
          </div>

          <div className="flex flex-col gap-4 md:gap-5">
            <div className="aspect-[16/7]">
              <HeroCard
                image={assets.midTop}
                eyebrow="Fast care for COVID, flu & strep"
                title="Get same-day testing & treatment"
                cta="Schedule now"
                href="https://www.walmart.com/cp/test-treat/4097505"
                variant="link"
                textColorClassName="text-[#001E60]"
                contentClassName="w-full md:w-[78%]"
                ariaLabel="Get same-day testing and treatment"
              />
            </div>

            <div className="grid grid-cols-1 gap-4 md:gap-5 sm:grid-cols-2">
              <div className="aspect-[3/4]">
                <HeroCard
                  image={assets.midBottomLeft}
                  title="Subscribe to your faves"
                  cta="Shop wellness"
                  href="https://www.walmart.com/browse/health/all-vitamins-supplements/976760_1005863_8090261"
                  variant="link"
                  textColorClassName="text-[#001E60]"
                  contentClassName="w-full"
                  ariaLabel="Subscribe to your faves"
                />
              </div>

              <div className="aspect-[3/4]">
                <HeroCard
                  image={assets.midBottomRight}
                  title="Now offering Mattress Accident Plans"
                  cta="Shop now"
                  href="https://www.walmart.com/browse/mattresses-accessories/4044_103150_539386_9682013"
                  variant="link"
                  textColorClassName="text-[#001E60]"
                  contentClassName="w-full"
                  ariaLabel="Mattress Accident Plans"
                />
              </div>
            </div>
          </div>

          <div className="lg:row-span-2">
            <div className="aspect-[3/5] lg:aspect-[3/6]">
              <HeroCard
                image={assets.right}
                eyebrow="Self-care delivered"
                title="Relax & reset in as fast as 1 hour*"
                cta="Shop now"
                href="https://www.walmart.com/cp/6545138"
                variant="link"
                textColorClassName="text-[#001E60]"
                contentClassName="w-full md:w-[70%]"
                ariaLabel="Self-care delivered"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
