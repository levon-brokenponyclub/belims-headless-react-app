...
export const Archive: React.FC<ArchiveProps> = ({
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (selectedFacetBrands.length > 0) {
             filtered = filtered.filter(p => p.brand && selectedFacetBrands.includes(p.brand));
        }
        ...
        return filtered;
    }, [products, category, brand, searchQuery, priceRange, sortBy, filterInStock, selectedFacetBrands]);
    ...
    const title = brand ? `${brand} Products` : (category ? category : (searchQuery ? `Search: "${searchQuery}"` : 'All Products'));
    ...
    return (
        <div className="shopify-section section-collection-template bg-white">
            ...
            {/* Breadcrumb Section */}
            <nav className="border-b border-gray-200 bg-gray-50 mb-8" aria-label="Breadcrumb">
                <div className="container mx-auto px-4 py-3">
                    <ol className="flex items-center space-x-2 text-sm text-gray-500">
                        <li>
                            <a href="#" className="hover:text-belims-accent transition-colors">Home</a>
                        </li>
                                    ...
                                    <details className="group" open>
                                        ...
                                        <div className="space-y-3 pt-2">
                                             <label className="flex items-center space-x-3 cursor-pointer group/item">
                                                ...
                                                <span className="text-gray-600 group-hover/item:text-gray-900 transition-colors">In Stock</span>
                                                <span className="text-gray-400 text-sm ml-auto">({products.filter(p => p.stock > 0).length})</span>
                                            </label>
                                            <label className="flex items-center space-x-3 cursor-pointer group/item">
                                                <div className="relative flex items-start">
                                                    <input 
                                                ...
                                                <span className="text-gray-600 group-hover/item:text-gray-900 transition-colors">On Sale</span>
                                            </label>
                                        </div>
                                    </details>
                                </div>
                                     ...
                                     <div className="border-b border-gray-200 pb-6">
                                        <details className="group">
                                            ...
                                            <div className="space-y-3 pt-2 max-h-48 overflow-y-auto">
                                                {uniqueBrands.map(b => (
                                                     <label key={b} className="flex items-center space-x-3 cursor-pointer group/item">
                                                        <div className="relative flex items-start">
                                                            <input 
                                                                type="checkbox" 
                                                                className="peer h-5 w-5 rounded border-gray-300 text-belims-accent focus:ring-belims-accent"
                                                        ...
                                                        <span className="text-gray-600 group-hover/item:text-gray-900 transition-colors">{b}</span>
                                                    </label>
                                                ))}
                                            </div>
                                        </details>
            ...
            {/* Mobile Filters Modal */}
             {mobileFiltersOpen && (
                <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex justify-end" onClick={() => setMobileFiltersOpen(false)}>
                    <div className="w-full max-w-xs bg-white h-full shadow-2xl p-6 overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-8">
                             {/* Mobile Price */}
                            <div>
                                <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-gray-500">Price</h4>
                            ...
                            {/* Mobile Availability */}
                            <div>
                                <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-gray-500">Availability</h4>
