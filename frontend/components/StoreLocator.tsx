...
import { MapPin, Navigation, CheckCircle, AlertTriangle, XCircle } from 'lucide-react';
...
export const StoreLocator: React.FC<StoreLocatorProps> = ({ currentStore, onSelectStore, onClose, checkingProduct }) => {
  ...
  const getStockStatus = (storeId: string) => {
    ...
    if (hash === 0) return { status: 'In Stock', color: 'text-green-600', icon: <CheckCircle size={14}/> };
    if (hash === 1) return { status: 'Low Stock', color: 'text-yellow-600', icon: <AlertTriangle size={14}/> };
    return { status: 'Out of Stock', color: 'text-red-500', icon: <XCircle size={14}/> };
  };
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-4 bg-belims-blue text-white flex justify-between items-center">
          <div>
             <h2 className="text-lg font-bold flex items-center gap-2 font-heading">
               <MapPin size={20} /> {checkingProduct ? 'Check Availability' : 'Find a Store'}
             </h2>
             {checkingProduct && <p className="text-xs text-blue-200 line-clamp-1 max-w-[300px]">{checkingProduct.name}</p>}
          </div>
        ...
        <div className="p-4 border-b bg-gray-50">
           ...
           <div className="text-xs text-gray-500 text-center cursor-pointer hover:underline" onClick={() => setSearchMode(searchMode === 'list' ? 'manual' : 'list')}>
             {searchMode === 'list' ? "Or enter a specific street address" : "Back to list"}
           </div>
        </div>
        
        {searchMode === 'manual' ? (
          <div className="p-6 text-center">
             <h3 className="font-bold text-gray-800 mb-2">Enter Delivery Address</h3>
             <input 
               type="text" 
             ...
             />
             <button className="bg-belims-blue text-white w-full py-2 rounded font-bold" onClick={() => {
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                         <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-600 font-bold">Open until 9PM</span>
                         {stock && (
                           <span className={`text-[10px] flex items-center gap-1 font-bold ${stock.color}`}>
                    ...
                    <div className="text-right">
                      <span className="text-xs font-bold text-gray-400 block">{store.distance} km</span>
                      {currentStore?.id === store.id && (
                        <span className="text-[10px] font-bold text-white bg-belims-blue px-2 py-0.5 rounded-full mt-2 inline-block">Selected</span>
                      )}
                    </div>
                  </div>
                </div>
